...
void main() {
  group('DerivationPath()', () {
    ...
    List<Uint8List> nodesList = [];

    setUp(() {
      for (int i = 0; i < 4; i++) {
        nodesList.add(Uint8List(i+1));
      }
      derivationPath = DerivationPath(nodesList: [nodesList[0], nodesList[1], nodesList[2]]);
    ...
    test('could add node path to its path', () {
      ...
      expect(derivationPath, [nodesList[0], nodesList[1], nodesList[2], nodesList[3]]);
    });
    ...
    test('could equal another paths with same value nodes', () {
      DerivationPath newDerivationPath = DerivationPath(nodesList: [nodesList[0], nodesList[1]]);
      DerivationPath sameDerivationPath = DerivationPath(nodesList: [nodesList[0], nodesList[1], nodesList[2]]);
